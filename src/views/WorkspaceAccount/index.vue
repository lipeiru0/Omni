...
const timezoneMenuOpen = ref<'profile' | null>(null)
...
function toggleTimezoneMenu(position: 'profile'): void {
...
function closeTimezoneMenu(): void {
  timezoneMenuOpen.value = null
}

function handleTimezoneMenuKeydown(event: KeyboardEvent): void {
  if (event.key === 'Escape') closeTimezoneMenu()
}

onMounted(async () => {
  document.addEventListener('click', closeTimezoneMenu)
  document.addEventListener('keydown', handleTimezoneMenuKeydown)
  syncProfileForm()
...
onBeforeUnmount(() => {
  ...
  document.removeEventListener('keydown', handleTimezoneMenuKeydown)
})
</script>
  ...
  <div class="workspace-shell">
    ...
    <main class="workspace-main" :class="{ loading }">
      ...
      <section class="content-card profile-card">
        ...
        <dl class="profile-list">
          <div>
            <dt>邮箱</dt>
            <dd>
              {{ user?.email || '—' }} <span v-if="isEmailVerified" class="verified">✓ 已验证</span
                ...
                >
                  ...
                  >
                    <span
                      ...
                      ><small>{{ item.value }} · {{ getTimezoneOffset(item.value) }}</small></span
                    >
        ...
        <div v-if="!isEmailVerified" ref="verificationPanel" class="email-panel">
          ...
          <div>
            ...
            <div class="email-actions">
              ...
              <button type="button" class="text-button" @click="openVerification">
                输入验证码
              </button>
            </div>
            <form
      ...
      <section class="content-card compact-card">
        ...
        <p v-if="loadingOAuthAccounts && oauthAccounts.length === 0" class="oauth-empty">
          正在读取绑定信息…
        </p>
      ...
      <section class="content-card security-card">
        <div>
          <h2>♢ 两步验证（2FA）</h2>
          <span :class="{ enabled: twoFactorEnabled }">{{
            loadingTwoFactorStatus ? '读取中…' : twoFactorEnabled ? '已启用' : '未启用'
          }}</span>
        </div>
        <p>
          在密码之外，再加一层来自验证器 App（Google Authenticator、Authy、1Password
          等）的一次性动态码。
        </p>
        ...
        <section v-if="backupCodes.length" class="backup-code-panel">
          ...
          <div class="security-actions">
            <button class="secondary-security-button" type="button" @click="copyBackupCodes">
              复制全部
            </button>
            <button class="primary-button" type="button" @click="finishBackupCodeStep">
              我已安全保存
            </button>
          </div>
        </section>
        ...
        <section v-else-if="twoFactorSetup" class="two-factor-setup">
          ...
          <div class="qr-box">
            <img v-if="twoFactorQrCode" :src="twoFactorQrCode" alt="两步验证配置二维码" />
          </div>
          <div class="manual-secret">
            <span>手动密钥</span><code>{{ twoFactorSetup.secret }}</code>
          </div>
          <label
            >输入验证器 App 显示的 6 位动态码
            <input
          ...
          <div class="security-actions">
            ...
            >
              {{ enablingTwoFactor ? '验证中…' : '验证并启用' }}
            </button>
            <button class="text-security-button" type="button" @click="cancelTwoFactorSetup">
              取消
            </button>
          </div>
        </section>
        ...
        <template v-else-if="!twoFactorEnabled">
          <button
            class="primary-button"
            type="button"
            :disabled="settingUpTwoFactor || loadingTwoFactorStatus"
            @click="beginTwoFactorSetup"
          >
            {{ settingUpTwoFactor ? '正在生成…' : '启用两步验证' }}
          </button>
        </template>

        <template v-else>
          <button
            v-if="!showDisableTwoFactor"
            class="danger-outline-button"
            type="button"
            @click="showDisableTwoFactor = true"
          >
            关闭两步验证
          </button>
          <form v-else class="disable-two-factor-form" @submit.prevent="submitDisableTwoFactor">
            ...
            <label
              >动态码或备份码<input
                v-model.trim="disableTwoFactorForm.code"
                type="text"
                autocomplete="one-time-code"
                placeholder="输入 6 位动态码或备份码"
            /></label>
            <div class="security-actions">
              <button class="danger-outline-button" type="submit" :disabled="disablingTwoFactor">
                {{ disablingTwoFactor ? '关闭中…' : '确认关闭' }}
              </button>
              <button
                class="text-security-button"
                type="button"
                @click="showDisableTwoFactor = false"
              >
                取消
              </button>
            </div>
      ...
      <section class="content-card danger-card">
        <template v-if="deletionPending">
          <h2>♙ 账号处于注销冷静期</h2>
          <p>
            账号和全部 API Key 当前已冻结。计划永久删除时间：<strong>{{
              formatDeletionDate(user?.deletionScheduledAt)
            }}</strong
            >。在此之前可以撤销注销并恢复使用。
          </p>
          <button type="button" :disabled="cancellingDeletion" @click="cancelDeletion">
            {{ cancellingDeletion ? '撤销中…' : '撤销账号注销' }}
          </button>
        </template>
        <template v-else>
          <h2>♙ 注销账号</h2>
          <p>
            注销后账号进入 30 天冷静期，全部 API Key
            立即冻结；期满将永久删除个人信息。冷静期内可随时撤销。
          </p>
          <button
          ...
          <form v-else class="deletion-confirmation" @submit.prevent="submitAccountDeletion">
            <label
              >此操作会立即冻结全部 API Key。请输入大写 <strong>DELETE</strong> 确认：
              <input
            ...
            <div class="security-actions">
              <button
                type="submit"
                :disabled="requestingDeletion || deletionConfirmation !== 'DELETE'"
              >
                {{ requestingDeletion ? '提交中…' : '确认申请注销' }}
              </button>
              <button class="text-security-button" type="button" @click="closeDeletionConfirmation">
                取消
              </button>
            </div>
          </form>
...
.header-tools > button,
.header-tools .account-menu > .avatar,
.header-tools .header-timezone > .compact-timezone-trigger,
.menu-button {
  padding: 0;
  border: 0;
  background: none;
...
.workspace-main {
  ...
  margin-left: max(310px, calc(50% - 280px));
...
.compact-timezone-trigger:hover,
.compact-timezone-trigger[aria-expanded='true'] {
  color: #504cf0;
}
.compact-timezone-trigger:disabled {
  opacity: 0.6;
  cursor: wait;
}
.timezone-globe {
...
.timezone-dropdown {
  ...
  border-radius: 8px;
  background: #fff;
  box-shadow: 0 8px 24px rgb(31 44 75 / 16%);
...
.header-timezone .timezone-dropdown {
  top: calc(100% + 25px);
  right: -70px;
...
.profile-timezone .timezone-dropdown {
  top: auto;
  right: 0;
...
.verification-form input:focus {
  border-color: #e29a00;
  box-shadow: 0 0 0 3px rgb(226 154 0 / 10%);
}
...
.oauth-card-heading h2 {
  margin-bottom: 20px;
}
.oauth-card-heading button {
  padding: 5px 0;
  border: 0;
  background: none;
  color: #5957e8;
...
.oauth-card-heading button:disabled {
  color: #9aa8c1;
  cursor: wait;
}
.oauth-empty {
...
.oauth-account-list li:first-child {
  border-top: 0;
}
.provider-badge {
  display: grid;
...
.security-card > div span {
  color: #98a8c4;
  font-size: 13px;
}
.security-card > div span.enabled {
  color: #16a169;
}
.security-card > p {
  margin: -10px 0 20px;
...
.two-factor-setup input:focus,
.disable-two-factor-form input:focus {
  border-color: #5957e8;
  box-shadow: 0 0 0 3px rgb(89 87 232 / 10%);
}
.security-actions {
  display: flex;
  gap: 14px;
...
.backup-code-panel {
  padding: 20px;
  border: 1px solid #f1c668;
  border-radius: 12px;
  background: #fff9e9;
}
.backup-code-panel h3,
.disable-two-factor-form h3 {
  margin: 0 0 10px;
  font-size: 15px;
}
.backup-code-panel ul {
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 8px 24px;
  padding: 14px 18px;
  border: 1px dashed #e6bd61;
  border-radius: 9px;
  background: #fff;
  list-style: decimal inside;
}
.backup-code-panel code {
  color: #2e3c55;
  font-size: 13px;
...
.deletion-confirmation input:focus {
  border-color: #ed5d43;
  box-shadow: 0 0 0 3px rgb(237 93 67 / 10%);
}
.deletion-confirmation .security-actions button {
  margin-top: 0;
}
.deletion-confirmation .text-security-button {
  background: none;
  color: #687d9f;
}
